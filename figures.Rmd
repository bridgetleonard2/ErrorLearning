...
p1 <- plot_ly(data = cleandata_agg_pred %>% filter(type=="Observed") %>% group_by(participant), x = ~cond, y = ~Accuracy, color = ~condition) %>%
  ...
  add_lines(color = I("#D9D6C7"), line = list(width = 0.3), split = ~participant,
            legendgroup="participants", name=~paste("Participant -", participant),
            showlegend=FALSE) %>%
  add_markers(data = cleandata_summary_pred %>% filter(type =="Observed") %>% full_join(Ose_data, by="condition") %>%  filter(condition=="error"), x = ~cond, y = ~Accuracy, error_y = list(type= "data", array = ~accuracy_se,color = '#e74c3c', thickness=2), type = "scatter", mode = "markers", marker = list(color = "#e74c3c", size = 12), legendgroup="average",
              name="Average - Error") %>% 
  add_markers(data = cleandata_summary_pred %>% filter(type =="Observed") %>% full_join(Ose_data, by="condition") %>%  filter(condition=="study"), x = ~cond, y = ~Accuracy, error_y = list(type= "data", array = ~accuracy_se,color = '#2ecc71', thickness=2), type = "scatter", mode = "markers", marker = list(color = "#2ecc71", size = 12), legendgroup="average",
              name="Average - Study") %>% 
...
p1 <- plot_ly(
  ...
  add_markers(
    data = cleandata_summary_pred %>% filter(type == "Observed") %>% full_join(Ose_data, by = "condition") %>% filter(condition == "error"),
    x = ~cond,
    y = ~Accuracy,
  ...
  add_markers(
    data = cleandata_summary_pred %>% filter(type == "Observed") %>% full_join(Ose_data, by = "condition") %>% filter(condition == "study"),
    x = ~cond,
    y = ~Accuracy,
  ...
  layout(
    margin = list(t = 75),
    title = list(text = "Final Test Accuracy", font = list(family = "Open Sans", size = 24, color = "#333")),
    xaxis = list(
      title = "Item Type",
