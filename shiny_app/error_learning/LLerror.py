...
def ll_participant(all_data, ppt, LL_data):
    data = all_data[all_data.participant == ppt]
    print(data)
    edecay, etemp, eter = opt.minimize(vLLelab, x0 = [0.5, 1, 1], args=(data,), method = "Powell", bounds=[[0.01, 2], [0, 2], [0.1, 2]]).x
    llelab = LLelabRT(data, edecay, etemp, eter)
        
    mdecay, mtemp, mter = opt.minimize(vLLmed, x0 = [0.5, 1, 1], args=(data,), method = "Powell", bounds=[[0.01, 2], [0, 2], [0.1, 2]]).x
