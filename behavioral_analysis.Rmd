...
```{r}
ACCdens <- ggplot(indiv_summary, aes(x=performance, fill=condition)) +
  geom_density(alpha=0.4) +
  scale_fill_manual(values=c("#74A57F", "#FFA69E"))

ACCdens

RTdens <- ggplot(indiv_summary, aes(x=avgRT, fill=condition)) +
  geom_density(alpha=0.4) +
  scale_fill_manual(values=c("#74A57F", "#FFA69E"))
...
ggplot(cleandata_agg, aes(x=condition, y=Accuracy, color=condition)) +
  ...
  # scale_color_aaas() +
  scale_color_manual(values=c("#FFA69E", "#74A57F")) +
...
```{r}
lldata <- read_csv("LLresults.csv")
```

```{r}
ggplot(lldata, aes(x=elab.LL, y=med.LL, color = best.model)) + geom_point(alpha = 0.7) +
  scale_color_manual(values=c("#c9a818", "#674b94")) +
  theme(legend.position="right") +
  ggtitle("Loglikelihood of Model Fit by Participant") +
  xlab("Elaborative Loglikelihood") +
  ylab("Mediator Loglikelihood")  + 
  labs(color = "Best Model") +
  theme_minimal()
```